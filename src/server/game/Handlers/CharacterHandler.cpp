...
void WorldSession::HandleCharEnumOpcode (WorldPacket & /*recv_data*/)
{
    ...
    m_charEnumCallback = CharacterDatabase.AsyncPQuery(!sWorld->getBoolConfig(CONFIG_DECLINED_NAMES_USED) ?
...
;        pos = 0;
    ...
    std::string IP_str = GetRemoteAddress();
    sLog->outChar("Account: %d (IP: %s) Login Character:[%s] (GUID: %u)",   GetAccountId(),   IP_str.c_str(),   pCurrChar->GetName(),   pCurrChar->GetGUIDLow());
...
void WorldSession::HandleCharRenameOpcode (WorldPacket& recv_data)
{
    ...
    m_charRenameCallback.SetFutureResult(CharacterDatabase.AsyncPQuery("SELECT guid,   name FROM characters WHERE guid = %d AND account = %d AND (at_login & %d) = %d AND NOT EXISTS (SELECT NULL FROM characters WHERE name = '%s')",   GUID_LOPART(guid),   GetAccountId(),   AT_LOGIN_RENAME,   AT_LOGIN_RENAME,   escaped_newname.c_str()));
}
...
void WorldSession::HandleSetPlayerDeclinedNames (WorldPacket& recv_data)
{
    ...
    trans->PAppend("INSERT INTO character_declinedname (guid,   genitive,   dative,   accusative,   instrumental,   prepositional) VALUES ('%u',   '%s',   '%s',   '%s',   '%s',   '%s')",   GUID_LOPART(guid),   declinedname.name[0].c_str(),   declinedname.name[1].c_str(),   declinedname.name[2].c_str(),   declinedname.name[3].c_str(),   declinedname.name[4].c_str());
...
void WorldSession::HandleAlterAppearance (WorldPacket & recv_data)
{
    ...
    //_player->ModifyMoney(-int32(Cost));          // it isn't free (KretEdit: Yes it is!)
...
void WorldSession::HandleCharFactionOrRaceChange (WorldPacket& recv_data)
{
    ...
    if (recv_data.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
    {
        ...
        for (std::map<uint32,   uint32>::const_iterator it = sObjectMgr->factionchange_achievements.begin(); it != sObjectMgr->factionchange_achievements.end(); ++it)
        {
            ...
            trans->PAppend("DELETE FROM `character_achievement` WHERE `achievement`=%u AND `guid`=%u",   team == BG_TEAM_ALLIANCE ? achiev_alliance : achiev_horde,   lowGuid);
            trans->PAppend("UPDATE `character_achievement` SET achievement = '%u' where achievement = '%u' AND guid = '%u'",   team == BG_TEAM_ALLIANCE ? achiev_alliance : achiev_horde,   team == BG_TEAM_ALLIANCE ? achiev_horde : achiev_alliance,   lowGuid);
        }
        ...
        for (std::map<uint32,   uint32>::const_iterator it = sObjectMgr->factionchange_items.begin(); it != sObjectMgr->factionchange_items.end(); ++it)
        {
            ...
            trans->PAppend("UPDATE `item_instance` ii,   `character_inventory` ci SET ii.itemEntry = '%u' WHERE ii.itemEntry = '%u' AND ci.guid = '%u' AND ci.item = ii.guid",   team == BG_TEAM_ALLIANCE ? item_alliance : item_horde,   team == BG_TEAM_ALLIANCE ? item_horde : item_alliance,   guid);
        }
        ...
        for (std::map<uint32,   uint32>::const_iterator it = sObjectMgr->factionchange_spells.begin(); it != sObjectMgr->factionchange_spells.end(); ++it)
        {
            ...
            trans->PAppend("DELETE FROM `character_spell` WHERE `spell`=%u AND `guid`=%u",   team == BG_TEAM_ALLIANCE ? spell_alliance : spell_horde,   lowGuid);
            trans->PAppend("UPDATE `character_spell` SET spell = '%u' where spell = '%u' AND guid = '%u'",   team == BG_TEAM_ALLIANCE ? spell_alliance : spell_horde,   team == BG_TEAM_ALLIANCE ? spell_horde : spell_alliance,   lowGuid);
        }
        ...
        for (std::map<uint32,   uint32>::const_iterator it = sObjectMgr->factionchange_reputations.begin(); it != sObjectMgr->factionchange_reputations.end(); ++it)
        {
            ...
            trans->PAppend("DELETE FROM character_reputation WHERE faction = '%u' AND guid = '%u'",   team == BG_TEAM_ALLIANCE ? reputation_alliance : reputation_horde,   lowGuid);
            trans->PAppend("UPDATE `character_reputation` SET faction = '%u' where faction = '%u' AND guid = '%u'",   team == BG_TEAM_ALLIANCE ? reputation_alliance : reputation_horde,   team == BG_TEAM_ALLIANCE ? reputation_horde : reputation_alliance,   lowGuid);
        }
    }
