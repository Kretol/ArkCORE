...
#ifndef ARKCORECORE_GAMEOBJECT_H
#define ARKCORECORE_GAMEOBJECT_H
...
enum LootState
{
    GO_NOT_READY = 0, GO_READY,          // can be ready but despawned, and then not possible activate until spawn
    GO_ACTIVATED, GO_JUST_DEACTIVATED
};
...
public:
    ...
    bool Create (uint32 guidlow, uint32 name_id, Map *map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit = 0, float scale = 0);
    ...
    GameObjectValue * GetGOValue () const
    {
        return m_goValue;
    }
